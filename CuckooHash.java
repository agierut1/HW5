	 ...
	 */
 	...
 	public void put(K key, V value) {
		...
		if (table[h1] != null && table[h1].getBucKey().equals(key) && table[h1].getValue().equals(value)) {
			return;
		} else if (table[h2] != null && table[h2].getBucKey().equals(key) && table[h2].getValue().equals(value)) {
			return;
		}
		if(get(key) == value) {
			return;
		} // three different ways of trying to not let the put method add an existing key + value and still doesn't work
		for(int i = 0; i < CAPACITY; i++) {
			if (table[h1] == null) {
				table[h1] = new Bucket<K,V>(key,value);
				return;
			}
				Bucket<K, V> move = table[h1];
				table[h1] = new Bucket<K, V>(key, value);
			if(table[h2] == null) {
				table[h2] = new Bucket<K, V>(move.bucKey, move.value);
				return;
			} else {
				Bucket<K, V> move2 = table[h2];
				table[h2] = new Bucket<K, V>(move.bucKey, move.value);
				rehash();
				put(move2.bucKey, move2.value);
			}
		}
	}
